...
class DeepFashionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        idx = list(self.keys)[i]
        imgs = list()
        stickmans = list()
        for num_man, man in enumerate(self.indices[idx]):
            if num_man == 2:
                break
            img = self.load_img("../deepfashion/" + man[0])
...
def get_train_loader(index_path, batch_size=3, random_seed=42, shuffle=True, resize_size=None):
    ...
    valid_dataset = DeepFashionDataset(index_path, transform=train_transform, shuffle=shuffle, train=False)
